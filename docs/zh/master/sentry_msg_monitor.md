...
该插件支持带外下电、OOM、panic、带内下电、UB内存故障和链路（link）事件消息接收和上报，依赖xalarmd服务。整体流程为：内核通知链阻塞事件进程并发送消息，消息经过内核设备、用户态插件、xalarmd的转发发送至对应的处理程序，程序处理完毕后将消息反向传播至内核，内核根据返回消息的时间和结果来判断下一步处理操作。
...
- 针对带外下电/oom/UB内存故障/panic/带内下电流程的劫持功能默认处于关闭状态，需要由管控面执行命令打开相应功能才能生效。链路事件同样默认处于关闭状态，需要由管控面执行命令开启才能生效。
...
| 链路（link）事件|1016 （ALARM_LINK_EVENT） |
...
| 链路（link）事件| msgid_{port_id:port_id,scna:scna,event:up/down}| msgid:uint64_t类型，表示消息对应的id<br>  port_id:uint16_t类型，表示发生链路状态变化的端口标识<br>  scna:unsigned int类型，表示发生链路状态变化的源CNA标识<br>  event:字符串类型，取值为"up"（链路恢复）或"down"（链路断开） |
