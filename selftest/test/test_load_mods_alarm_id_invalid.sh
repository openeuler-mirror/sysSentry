...
tmp_log="tmp_log"
start_line=0
mod_name="test_alarm_id_invalid"
...
function do_test() {
    ...
    invalid_list=("abcd" " " "0x10")
    for arg in "${invalid_list[@]}"; do
        start_line=$(expr $(wc -l < /var/log/sysSentry/sysSentry.log) + 1)

        sed -i "s/^alarm_id.*/alarm_id=${arg}/g" "/etc/sysSentry/tasks/$mod_name.mod"
        ...
        expect_true "grep -E 'mod $mod_name: alarm_id must be an integer' ${tmp_log}" || cat ${tmp_log}
    done

    invalid_list=("-1" "0" "1" "1000" "1129" "10000")
    for arg in "${invalid_list[@]}"; do
        start_line=$(expr $(wc -l < /var/log/sysSentry/sysSentry.log) + 1)
        ...
        sed -n "${start_line}, ${end_line}p" /var/log/sysSentry/sysSentry.log > ${tmp_log}
        expect_true "grep -E 'mod $mod_name: alarm_id must be in' ${tmp_log}" || cat ${tmp_log}
    done
}
